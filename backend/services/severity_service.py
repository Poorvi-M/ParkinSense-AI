...
def get_user_severity_records(user_id: int, db: Session) -> list[Severity]:
    """
    Return all Severity records for a given user, ordered newest first.

    Ordering by created_at DESC surfaces the most recent clinical snapshot
    at index 0, which is the expected default for monitoring dashboards.
    ...
    Returns:
        A list of Severity ORM objects ordered by created_at descending.
        Returns an empty list if the user has no severity records.
    """
    return (
        db.query(Severity)
