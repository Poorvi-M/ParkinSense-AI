...
def create_severity_route(
    ...
    return SeverityResponse.model_validate(
        create_severity(
...
def get_my_severities_route(
    ...
    """
    severities = get_user_severities(user_id=current_user.id, db=db)
    return [SeverityResponse.model_validate(s) for s in severities]
...
def get_all_severities_route(
    ...
    """
    severities = get_all_severities(db=db)
    return [SeverityResponse.model_validate(s) for s in severities]
