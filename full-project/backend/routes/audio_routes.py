...
def upload_audio_route(
    ...
    return AudioFileResponse.model_validate(result)
# GET /audio/me
...
def get_my_audio_route(
    ...
    """
    results = get_user_audio_files(
        user_id=current_user.id,
        db=db,
    )

    return [AudioFileResponse.model_validate(a) for a in results]
