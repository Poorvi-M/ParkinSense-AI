...
_AUDIO_MAGIC: list[bytes] = [
    b"RIFF",        # WAV  (bytes 0-3; bytes 8-11 are "WAVE" but RIFF is sufficient)
    b"ID3",         # MP3 with ID3v2 tag
    b"\xff\xfb",    # MP3 MPEG-1 Layer 3, no padding
    b"\xff\xf3",    # MP3 MPEG-2 Layer 3
    b"\xff\xf2",    # MP3 MPEG-2 Layer 3
    b"\xff\xe3",    # MP3 MPEG-2.5 Layer 3
    b"\xff\xfa",    # MP3 MPEG-1 Layer 3, no padding (CBR)
    b"\xff\xf9",    # MP3 MPEG-2 Layer 3 (CBR)
]
...
def _validate_audio_file(file: UploadFile) -> str:
    ...
    Steps:
        1. Check the file extension against the allowed set.
        2. Read the first 12 bytes and compare against known audio magic bytes.
           Rewinds the stream to position 0 after reading so subsequent
           chunk-streaming in _save_file_to_disk sees the full file.

    Raises:
        HTTP 400 -- if the extension is not .wav or .mp3
        HTTP 400 -- if the file content does not match a known audio signature
    """
    ...
    # Read magic bytes and rewind — _save_file_to_disk must see the full stream
    header = file.file.read(_MAGIC_PEEK_BYTES)
    file.file.seek(0)

    if not any(header.startswith(magic) for magic in _AUDIO_MAGIC):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="File content does not match a valid audio format. "
                   "Only genuine .wav and .mp3 files are accepted.",
        )
...
def get_user_audio_files(user_id: int, db: Session) -> list[AudioFile]:
    """
    Return all AudioFile records for a given user, newest first.
    """
    return (
        db.query(AudioFile)
...
def update_audio_status(audio_id: int, new_status: UploadStatus, db: Session) -> AudioFile:
    """
    Update the upload_status of an AudioFile. Returns the updated object.
    """
    ...
    audio.upload_status = new_status
    db.commit()
    db.refresh(audio)
