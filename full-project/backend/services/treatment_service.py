...
def get_all_treatments(db: Session) -> list[Treatment]:
    """
    Return all Treatment records across all users, ordered newest first.
    Role enforcement is the responsibility of the route layer.
    """
    return (
        db.query(Treatment)
        .order_by(Treatment.created_at.desc())
