...
def startup() -> None:
    """
    Initialize database tables and assert required runtime invariants.
    """
    if settings.SECRET_KEY in _WEAK_SECRETS:
        raise RuntimeError(
            "SECRET_KEY is set to a placeholder value. "
            "Set a strong random secret in your .env before starting the server."
        )

    if settings.DEBUG:
        warnings.warn(
            "DEBUG=True is enabled. SQL queries and internal details will be "
            "logged to stdout. Do NOT run with DEBUG=True in production.",
            stacklevel=1,
        )
        logger.warning(
            "Server started with DEBUG=True. "
            "Disable this in production by setting DEBUG=False in .env."
        )
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=_ALLOWED_ORIGINS,        # Never "*" when credentials are sent
